...
#			ifdef _TTNSERVER
			...
			// [DB] Guard: skip send if buildPacket returned nothing useful
			if (build_index <= 12) {
#				if _MONITOR>=1
				if (debug>=0) { mPrint("[DB] buildPacket bad index=" + String(build_index) + ", dropping"); }
#				endif //_MONITOR
			}
			// [DB] Guard: skip send if ttnServer is unresolved (0.0.0.0)
			else if (ttnServer.toString() == "0.0.0.0" || ttnServer.toString() == "0:0:0:0") {
#				if _MONITOR>=1
				if (debug>=0) { mPrint("[DB] ttnServer unresolved - queuing packet"); }
#				endif //_MONITOR
				appendGwQueue(buff_up, (uint16_t)build_index);
			}
			// [DB] Probe ChirpStack before attempting sendUdp().
			//
			// UDP is fire-and-forget — sendUdp() returns true even when ChirpStack
			// is completely offline (power failure, Pi still booting, service down).
			// The TCP probe on port 8080 gives a real up/down signal so we can
			// queue the packet to SPIFFS instead of silently losing it.
			//
			// Flow:
			//   probe FAIL  -> ChirpStack offline -> queue packet, skip sendUdp()
			//   probe OK    -> ChirpStack online  -> sendUdp()
			//     sendUdp FAIL -> network blip -> queue + invalidate probe cache
			//     sendUdp OK   -> success -> opportunistic drain of one queued packet
			else if (!isChirpStackReachable()) {
#				if _MONITOR>=1
				if (debug>=0) { mPrint("[GWDB] ChirpStack OFFLINE (probe failed) - queuing packet"); }
#				endif //_MONITOR
				appendGwQueue(buff_up, (uint16_t)build_index);
			}
			else if (!sendUdp(ttnServer, _TTNPORT, buff_up, build_index)) {
				// Probe passed but sendUdp failed — transient fault, invalidate cache
#				if _MONITOR>=1
				if (debug>=0) { mPrint("[GWDB] sendUdp failed after probe OK - queuing, cache invalidated"); }
#				endif //_MONITOR
				csProbeInvalidate();
				appendGwQueue(buff_up, (uint16_t)build_index);
			} else {
				// [DB] Forward succeeded - opportunistically drain one queued packet
#				if _MONITOR>=1
				if (debug>=1) { mPrint("[GWDB] Forwarded to ChirpStack OK"); }
#				endif //_MONITOR
				tryForwardGwQueued();
			}
#			endif //_TTNSERVER

			yield();									// make sure the kernel sends message to server asap
...
#	endif //_LOCALSERVER
			...
			// [DB] Decode 18-byte epoch-first payload and write to gwlog.csv
#			if _LOCALSERVER>=1
			{
				uint32_t gwEpoch = 0;
				float gwMoist=0, gwTemp=0, gwHum=0, gwPres=0, gwD1=0, gwD2=0, gwDist=0;
				if (decodeGwPayload(statr[0].data, statr[0].datal,
									gwEpoch, gwMoist, gwTemp, gwHum, gwPres, gwD1, gwD2, gwDist)) {
					String gwRow = buildGwCsvRow(gwEpoch, gwMoist, gwTemp, gwHum, gwPres, gwD1, gwD2, gwDist);
					appendGwLog(gwRow);
#					if _MONITOR>=1
					if (debug>=1) {
						mPrint("[GWDB] Logged: " + gwRow);
					}
#					endif //_MONITOR
				} else {
#					if _MONITOR>=1
					if (debug>=1) {
						mPrint("[GWDB] Payload too short (need 18, got datal=" + String(statr[0].datal) + ")");
					}
#					endif //_MONITOR
				}
			}
#			endif //_LOCALSERVER>=1
