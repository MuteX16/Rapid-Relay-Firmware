...
extern int gwQueueCount;													// _database.ino
void fsGwInit();															// _database.ino
void tryForwardGwQueued();													// _database.ino
void tickGwRetry();															// _database.ino
...
#	if _DUSB>=1
		...
		Serial.setTimeout(10);								// [DB] 10ms timeout prevents readStringUntil blocking stateMachine
		delay(500);
		Serial.flush();
#	endif //_DUSB
...
#	endif //_MONITOR
	...
	//
	// [DB] Init gateway log/queue files on SPIFFS (must be after SPIFFS.begin())
	fsGwInit();
...
#	endif //_WIFIMANAGER
	...
	delay(100);												// Let WiFi mode settle before reading MAC
...
#	ifdef _TTNSERVER
		// [DB] Use fromString() for raw IP addresses (e.g. "192.168.1.8").
		// WiFi.hostByName() on a raw IP string can return 0.0.0.0 on ESP32,
		// making every sendUdp() fail silently even when ChirpStack is reachable.
		if (!ttnServer.fromString(_TTNSERVER)) {
			// Not a raw IP - resolve as hostname via DNS
			ttnServer = resolveHost(_TTNSERVER, 10);
			if (ttnServer.toString() == "0.0.0.0" || ttnServer.toString() == "0:0:0:0") {
#				if _MONITOR>=1
				if (debug>=1) { mPrint("setup:: TTN Server not found, retrying..."); }
#				endif
				delay(5000);
				ttnServer = resolveHost(_TTNSERVER, 10);
			}
...
#		if _MONITOR>=1
		if (debug>=0) { mPrint("setup:: ttnServer IP=" + ttnServer.toString()); }
#		endif
		delay(100);
#	endif //_TTNSERVER
...
#			endif //_MONITOR
	...
	handleGwSerialCmd();									// [DB] After stateMachine — 10ms Serial timeout set in setup()

	// [DB] Offline retry backoff — probes ChirpStack on an exponential schedule
	// and drains queued packets in bursts when it comes back online.
	// Non-blocking: returns immediately when queue is empty or interval not reached.
#ifdef _TTNSERVER
	tickGwRetry();
#endif
...
#		ifdef _TTNSERVER
		// [DB] Re-resolve ttnServer if it is unset (0.0.0.0).
		// This handles the case where ChirpStack was offline at boot or
		// the gateway lost its IP assignment after a network change.
		if (ttnServer.toString() == "0.0.0.0" || ttnServer.toString() == "0:0:0:0") {
			if (!ttnServer.fromString(_TTNSERVER)) {
				ttnServer = resolveHost(_TTNSERVER, 3);
			}
#			if _MONITOR>=1
			if (debug>=0) { mPrint("[DB] Re-resolved ttnServer: " + ttnServer.toString()); }
#			endif
		}
#		endif //_TTNSERVER
		...
		// [DB] Periodic queue drain — attempt to forward packets that were
		// queued while the Pi (ChirpStack) was offline. Runs every _PULL_INTERVAL
		// seconds (16s). Drains up to 3 entries per tick so the loop is not
		// blocked; if sendUdp() fails the retry stops immediately (Pi still down).
#		ifdef _TTNSERVER
		if (gwQueueCount > 0) {
#			if _MONITOR>=1
			if (debug>=0) { mPrint("[GWDB] Periodic retry: " + String(gwQueueCount) + " queued packets remaining"); }
#			endif //_MONITOR
			tryForwardGwQueued();
			tryForwardGwQueued();
			tryForwardGwQueued();
